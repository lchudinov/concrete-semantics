theory Exercise83 imports Big_Step Star
begin

subsection "This exercise not finished"
...
datatype instr = 
  LOADI int | LOAD addr | ADD | STORE addr |
  JMP int | JMPLESS int | JMPGE int

type_synonym stack = "val list"
type_synonym mstate = "addr \<Rightarrow> val"
type_synonym mmap = "vname \<Rightarrow> addr"
type_synonym config = "int \<times> mstate \<times> stack"
...
value "bcomp (Andb (Bc True) (Bc True)) False 3"

value "bcomp (Bc True) True 3"
value "bcomp (Bc False) False 1"

value "bcomp (Andb (Bc False) (Bc True)) True 3"

value
  "bcomp (Andb (Less (V ''x'') (V ''y'')) (Not(Less (V ''u'') (V ''v''))))
     False 3"

value
  "bcomp (Andb (Bc False) (Not(Less (V ''u'') (V ''v''))))
     False 3"
...
next
  case (Andb b1 b2)
  from Andb(1)[of "if f then size(bcomp b2 f n) else size(bcomp b2 f n) + n" "False"] 
       Andb(2)[of n f]
       Andb(3) 
