...
IfNone:  "bval b s = None \<Longrightarrow> (IF b THEN c1 ELSE c2, Some s) \<Rightarrow> None" |
IfTrue:  "\<lbrakk>bval b s = Some True;  (c1,Some s) \<Rightarrow> s'\<rbrakk> \<Longrightarrow> (IF b THEN c1 ELSE c2, Some s) \<Rightarrow> s'" |
IfFalse: "\<lbrakk>bval b s = Some False; (c2,Some s) \<Rightarrow> s'\<rbrakk> \<Longrightarrow> (IF b THEN c1 ELSE c2, Some s) \<Rightarrow> s'" |
...
WhileTrue: "\<lbrakk>bval b s = Some True; (c, Some s) \<Rightarrow> s'; (WHILE b DO c,s') \<Rightarrow> s''\<rbrakk>  \<Longrightarrow> (WHILE b DO c, Some s) \<Rightarrow> s''"
