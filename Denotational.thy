...
definition W :: "(state \<Rightarrow> bool) \<Rightarrow> com_den => (com_den \<Rightarrow> com_den)" where
...
"D (c1;;c2) = D(c1) O D(c2)" |
"D (IF b THEN c1 ELSE c2) = {(s,t). if bval b s then (s,t) \<in> D c1 else (s,t) \<in> D c2}" |
